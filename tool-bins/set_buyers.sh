...
export ORG_CONTEXT=buyers
MSP_ID="$(tr '[:lower:]' '[:upper:]' <<< ${ORG_CONTEXT:0:1})${ORG_CONTEXT:1}"
export ORG_NAME=$MSP_ID

# Added this Oct 22
export CORE_PEER_LOCALMSPID="BuyersMSP"

# Logging specifications
export FABRIC_LOGGING_SPEC=INFO

# Location of the core.yaml
export FABRIC_CFG_PATH=/workspaces/config/buyer

# Address of the peer
export CORE_PEER_ADDRESS=teraconsortium-buyers:7051

# Local MSP for the admin - Commands need to be executed as org admin
export CORE_PEER_MSPCONFIGPATH=/workspaces/config/crypto-config/peerOrganizations/buyers.tera.bt/users/Admin@buyers.tera.bt/msp
